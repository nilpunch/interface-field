...
namespace InterfaceField.Editor
{
	...
	public class ProxyDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			var proxyType = fieldInfo.GetCustomAttribute<ProxyAttribute>().ProxyType;

			if (!interfaceType.IsAssignableFrom(proxyType))
			{
				Debug.LogError($"Provided proxy type {proxyType.GetFullGenericName()} is not assignable from {interfaceType.GetFullGenericName()}!");
				return;
			}

			if (property.boxedValue == null || !proxyType.IsInstanceOfType(property.boxedValue))
			{
				property.boxedValue = Activator.CreateInstance(proxyType);
				property.serializedObject.ApplyModifiedPropertiesWithoutUndo();
			}

			var objectField = property.FindPropertyRelative("_object");
